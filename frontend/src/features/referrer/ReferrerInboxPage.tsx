  ...
  return (
    ...
    >
      <Group gap="sm" wrap="nowrap">
        <Avatar size={36} src={other?.avatar_url || undefined} radius="xl" color="teal">
        ...
        <Stack gap={2} style={{ flex: 1, minWidth: 0 }}>
          <Group gap={4} wrap="nowrap">
            ...
            {!lead.is_connected && lead.pro && (
              <Badge size="xs" color="gray" variant="outline">Not connected</Badge>
            )}
          </Group>
    ...
    <Stack h="100%" gap={0} style={{ minHeight: 0 }}>
      ...
      <Box p="md" style={{ borderBottom: "1px solid var(--gk-border)", background: "var(--gk-bg-surface)" }}>
        <Group justify="space-between" wrap="nowrap">
          <Group gap="sm" wrap="nowrap">
            <Avatar size={40} src={other?.avatar_url || undefined} radius="xl" color="teal">
              {!other?.avatar_url && (other?.name?.[0]?.toUpperCase() ?? "?")}
            </Avatar>
            <Stack gap={2}>
              <Group gap={6}>
          ...
          {messages.map((msg) => (
            ...
            >
              ...
              >
                ...
                <Text size="xs" opacity={0.6} mt={2}>{relTime(msg.created_at)}</Text>
              </Box>
            </Box>
          ))}
          <div ref={bottomRef} />
      ...
      {canChat ? (
        ...
        <Box p="sm" style={{ borderTop: "1px solid var(--gk-border)", textAlign: "center" }}>
          <Text size="xs" c="dimmed">
            {isArchived ? "This conversation is archived." : "This conversation is closed."}
          </Text>
        </Box>
      )}
    </Stack>
...
function ComposeModal({
  ...
  function handleClose() {
    setHandle(""); setBody(""); setError(null); onClose();
  }
...
// ── Main page ─────────────────────────────────────────────────────────────────
export function ReferrerInboxPage() {
  ...
  // Open lead from URL on mount
  useEffect(() => {
    const id = leadId ? Number(leadId) : NaN;
    if (!isNaN(id) && id > 0) {
      getLead(id).then((lead) => {
        setThreads((prev) => prev.some((t) => t.id === id) ? prev : [lead, ...prev]);
        setSelectedId(id);
      }).catch(() => {});
    }
  }, [leadId]);
  ...
  function handleSelect(lead: InboxLead) {
    ...
    navigate(`/us/${slug}/inbox/${lead.id}`, { replace: true });
  }
      ...
      >
        ...
        >
          ...
          >
            ...
            <Tooltip label="Compose new message" withArrow>
              <ActionIcon size="sm" variant="light" color="teal" onClick={() => setComposeOpen(true)}>
                <IconEdit size={14} />
              </ActionIcon>
            </Tooltip>
          </Box>
          ...
          <ScrollArea style={{ flex: 1 }} p="xs">
            {activeTab === "feedback" ? (
              ...
              <GkEmptyState
                ...
                description={
                  activeTab === "lead" ? "Quote requests from pros appear here."
                  : activeTab === "chat" ? "Direct conversations appear here."
                  : activeTab === "request" ? "Unsolicited messages appear here."
                  : "Messages you compose appear here."
                }
              />
            ) : (
              <Stack gap={2}>
        ...
        <Box style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          {activeTab === "feedback" ? (
            ...
            <Stack align="center" justify="center" h="100%" gap="sm">
              <IconInbox size={32} color="var(--gk-accent-primary)" />
